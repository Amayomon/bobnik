...
export function StatsBar({ members, getCountInRange, getAllTimeCount, getStreak }: StatsBarProps) {
  ...
  // Average per day (last 7 days, all members)
  ...
  const stats = [
    {
      icon: '🏆',
      label: 'ŽEBŘÍČEK',
      value: `1. ${weeklyLeader.name} ${weeklyLeader.emoji}`,
    },
    {
      icon: '📅',
      label: 'TÝDENNÍ SOUČET',
      value: `${weeklyTotal} Bobníků`,
    },
    {
      icon: '📊',
      label: 'PRŮMĚR / DEN',
      value: `${avgPerDay} denně`,
    },
    {
      icon: '🔥',
      label: 'NEJDELŠÍ STREAK',
      value: `${bestStreak} dní v kuse`,
    },
  ];
  ...
  return (
    <div className="grid grid-cols-2 gap-2 p-3 bg-stats-bg rounded-xl">
      {stats.map((stat, i) => (
        <div
          key={i}
          className="flex flex-col items-center text-center p-2.5 bg-card rounded-lg"
        >
